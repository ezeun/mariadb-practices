...
select emp_no, dept_no
	from dept_emp
    where dept_no = 'd005'
    or dept_no = 'd009'; -- where dept_no in ('d005','d009')

-- like 검색
-- 예제8: employee 테이블에서 1989년에 입사한 직원의 이름, 입사일 출력
select first_name, gender, hire_date
	from employees
    where '1991-01-01' <= hire_date 
    and hire_date <= '1989-12-31';

select first_name, gender, hire_date
	from employees
    where hire_date between '1991-01-01' and '1989-12-31';
...
-- 예제11: 직원의 사번과 월급을 사번(asc), 월급(deac) 순으로 출력
select emp_no, salary, from_date, to_date
	from salaries
